  ...
  return (
    ...
    >
      <div className="list-item">
        <Sprite id={id} alt={" "} height={96} width={96} />

        <div className="pokemon-name">
          {capitalize(pkData[id]?.pokemon?.identifier ?? "")}
        </div>

        {windowWidth >= 1000 && (
          <div className="abilities-container">
            {data?.abilities?.map?.((row) => (
              <div key={Object.entries(row).toString()}>
                {capitalize(
                  otherData.abilities?.[row.ability_id]?.identifier
                ) || ""}
              </div>
            ))}
          </div>
          ...
          <div className="stats-container">
            {data?.stats?.map((stat) => (
              <div key={stat.stat_id}>
                <span style={{ opacity: 0.35, marginRight: 4 }}>
                  {['', "hp", "atk", "def", "spa", "spd", "spe"][stat.stat_id]}
                </span>
                {stat.base_stat}
              </div>
            ))}
        ...
        <div className="types-container">
          {data?.types?.map?.((type) => (
            <Type type={type?.type_id ?? 0} key={type?.type_id} />
          )) || ""}
        </div>
      </div>
    </div>
